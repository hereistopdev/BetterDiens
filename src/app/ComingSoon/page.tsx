...
const stateData: string[] = ["California", "New York", "Washington", "Texas", "Arizona"];
...
export default function ComingSoon() {
  ...
  async function handleClickNotify() {
    ...
    const bodyData: RequestNotifyPropsType = { email, state: selectedState, isSeller };
    console.log(bodyData);

    try {
      await axios.post("https://fd30-107-155-105-218.ngrok-free.app/api/client/create", bodyData);
    ...
    <div className="relative flex w-full h-screen text-white">
      ...
      <div className="absolute w-full flex flex-col text-center px-4 sm:px-6">
        <div className="relative z-10 p-3">
          ...
          <p className="font-bold font-[family-name:var(--font-exo)] text-[48px] text-black leading-14">Local independent retailers <br /> offering more than convenience</p>
          <div className="flex justify-center m-[20px]">
            <p className="w-[670px] text-black leading-8 font-[family-name:var(--font-roboto)] text-[24px]">Website is coming soon. Get a chance tp get <span className="font-bold">xaClub </span>membership (<span className="font-bold">$30 value</span>) free for one year, which offers bigger deals and early access to promotions
              (<span className="font-bold underline">first 100</span> who sign-up automatically win this deal).</p>
          </div>
          <div className="mt-6 flex flex-col sm:flex-row md:flex-row 2xl:flex-row justify-center">
            <div className="flex flex-col sm:flex-row">
              ...
              <div className="relative sm:w-auto mt-4 sm:m-0">
                <CustomSelect placeholder="Are you a Seller?" selectedValue={selectedSeller} setSelectedValue={setSelectedSeller} optionData={isSellerData} customStyle={"sm:w-50"} />
              </div>
              <div className="relative sm:w-auto mt-4 sm:m-0">
                <CustomSelect placeholder="State" selectedValue={selectedState} setSelectedValue={setSelectedState} optionData={stateData} />
              </div>
            </div>
            <button
              type="button"
      ...
      <CustomAlert content="Created Successfully!" severity="success" open={successAlertOpen} setOpen={setSuccessAlertOpen} />
      <CustomAlert content="Internal Server Error!" severity="error" open={failedAlertOpen} setOpen={setFailedAlertOpen} />
    </div >
  );
}
